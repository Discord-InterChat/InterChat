 ...
 */
export const checkBlacklists = async (client: Client, hubId: string, guildId: string, userId: string) => {
  const { userBlacklists, serverBlacklists } = client;
  const userBlacklisted = await userBlacklists.fetchBlacklist(hubId, userId);
  const guildBlacklisted = await serverBlacklists.fetchBlacklist(hubId, guildId);
