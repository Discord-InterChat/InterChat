...
module.exports = {
	...
	*/
	async execute(interaction) {
		...
		const reportedUser = await interaction.client.users.fetch(args.embeds[0].footer.text.split('┃')[-1]);
		const reportedServer = await interaction.client.guilds.fetch(args.embeds[0].author.url.split('/')[-1]);

		const reportsChannel = await interaction.client.channels.fetch('976099718251831366'); // FIXME: change channelId to 821610981155012628 later
