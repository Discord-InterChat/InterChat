	...
	async messageTypes(client, message, channelObj, embed, setupDb) {
		...
		const webhookAutomate = async (chan) => {
			try {
				...
				await webhook.send({ content: message.content, username: message.author.username, avatarURL: message.author.avatarURL(), files: [message.attachments.first().proxyURL] });
				logger.info('semt');
			}
			catch (error) {
		...
		else if (channelInDB && channelInDB.isEmbed === false && allChannel == channelInDB.channelId) {
